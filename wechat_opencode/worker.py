...
class WorkerManager:
    ...
    def start_task(self, task: str) -> bool:
        ...
        sys_prompt = (
            ...
            "═══ 屏幕查看能力 ═══\n"
            "你可以通过截图+OCR 来\"看\"屏幕上的内容。当你需要确认桌面/窗口/应用状态时:\n"
            "  1. 执行: python -c \"from wechat_opencode.screenshot import describe_screen; print(describe_screen())\"\n"
            "  2. 读取返回的文字内容，了解屏幕上有什么\n"
            "  3. 根据识别结果决定下一步操作\n"
            "  场景举例:\n"
            "    · \"桌面上有什么文件\" → 截图看桌面\n"
            "    · \"豆包窗口上显示什么\" → 截图识别豆包窗口内容\n"
            "    · \"确认文件下载完成了吗\" → 截图看浏览器下载栏\n\n"
            "═══ 验证清单 ═══\n"
            "每步执行后必须验证，用工具实际操作:\n"
